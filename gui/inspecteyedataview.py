...
class SignalPlotWidget(FigureCanvas):
    ...
    # plot those first.
    def plotFixations(
            self, fix, xgaze, ygaze, time,
            startfixstyle=_startf, endfixstyle=_endf):
        self.axgazex.plot(
    ...
    def plotSaccades(
        ...
        # mark x gaze
        self.axgazex.plot(
            times[boolvecstart], xgaze[boolvecstart], startsacstyle
        )
        self.axgazex.plot(times[boolvecend], xgaze[boolvecend], endsacstyle)

        # mark y gaze
        self.axgazey.plot(
            times[boolvecstart], ygaze[boolvecstart], startsacstyle
        )
...
class GazePlotWidget(FigureCanvas):
    ...
    def plotGazePicture(
        ...
        if img is None:
            msg = "Unable to load: " + picture + \
                "Did you set the stimulus directory?"
            p = self
            last = None
            while p:
                last = p
...
class TabbedSignalView(dv.CustomDataView, QtWidgets.QWidget):
    ...
    def _initGui(self):
        # create a layout without spacing
    ...
    ##
    # One tab contains a vertical box with a FigureCanvas and a Matplotlib
    # toolbar.
    #
    # This function helps creating such a widget with embedded Matplotlib
    # canvas and toolbar
    # in a QVBoxLayout.
    def _createPlotVBox(self, widget, toolbar):
        ...
        layout.addWidget(toolbar, 0, QtCore.Qt.AlignLeft)
        w.setLayout(layout)
        return w
...
##
# Display the EyeData of a number of files
#
# The InspectEyeDataView is a widget that helps inspecting
...
# Therefore this widget is mostly helpful in examining the quality
# of the data. Also it is helpful in determining the most proper
...
class InspectEyeDataView(dv.DataView):
    ...
    def __init__(self, model, controller, parent=None):
        ...
        if not self.hasValidData():
            model.getMainWindow().reportStatus(
                sm.StatusMessage.error, "Invalid Data..."
            )
