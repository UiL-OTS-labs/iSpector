...
class StimulusWidget(QtWidgets.QWidget):
    ...
    def __init__(self,
        ...
        ## the height of the screen of the eyetracker
    ...
    #
    # the reset background color takes effect after new paint event, so
    ...
    def _default_img_transform(self, painter):
        ...
        try:
            if scrheight <= 0 or scrwidth <= 0:
                raise RuntimeError(
                    repr(self) + "Unable to determine Screensize"
                )
    ...
    def paintEvent(self, event):
        ...
        # Apply transformations that center the stimulus nicely and completely
    ...
    # will call this function. Derived classes should not overload the
    # paintEvent, but this method. Then the StimulusWidget will draw the
    # background while the derived class paints the relevant foreground
